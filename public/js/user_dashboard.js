...
async function editExpense(e) {
    try {
        ...
        if (e.target.classList.contains("edit")) {
            ...
            const res = await axios.get(
                "http://localhost:3000/expense/getAllExpenses" // here not need of api call we can acess all the data from tr.children[1] tr.children[2].... so on
                // ,{ headers: { Authorization: token } }
            );
                ...
                if (expense.id == id) {
                    ...
                    addExpenseBtn.addEventListener("click", async function update(e) {
                        ...
                        const res = await axios.post(
                            `http://localhost:3000/expense/updateExpense/${id}`,
                            {
...
async function buyPremium(e) {
    // const token = localStorage.getItem("token");
    const res = await axios.get(
        "http://localhost:3000/user/premiumMembership"
        // ,{ headers: { Authorization: token } }
    );
